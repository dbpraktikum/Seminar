...
\begin{frame}
	...
	\begin{alertblock}{Steiner Baum}
		Sei $G$ ein ungerichteter Graph mit $n$ Knoten und $K\subseteq V(G)$ von Endpunkten (terminals). Ein Steiner Baum für $K$ in $G$ ist ein zusammenhängender Teilgraph $H$ von $G$, der $K$ enthält ($K\subseteq H$)
	\end{alertblock}
...
\begin{frame}
	...
	\begin{alertblock}{Theorem 6.3}
		Steiner Baum kann in Zeit $3^{K}n^{\mathcal{O}(1)}$
	\end{alertblock}
\end{frame}
%---------------------------------------------------------------------
