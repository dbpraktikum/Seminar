...
%==== Informationen ====
	...
	\institute[Parametrisierte Algorithmen]{Parametrisierte Algorithmen Seminar}
	\date{16.\,11.\,2015}
	%Titelgraphic
...
\begin{frame}
	...
	\begin{itemize}
		...
		\pause
		\item $F_j\in\mathcal{F}'$, dann ist $\mathcal{F}'\backslash F_j$ eine zulässige Lösung für $T[X\backslash F_j, j-1]$ 
	\end{itemize}
	\pause	
	\item Für $\leq$:
	\pause
	\begin{itemize}
		\item Eine Lösung $\mathcal{F}'$ für $T[X,j-1]$ ist auch eine zulässige Lösung für $T[X,j]$
		\pause
		\item Für eine Lösung  $\mathcal{F}'$ für $T[X\backslash F_j, j-1]$ gilt, dass $\mathcal{F}'\cup F_j$ eine zulässige Lösung für $T[X,j]$ ist.
	\end{itemize}	 	
	\end{itemize}
\end{frame}
...
%	\item Für $\leq$:
%	\begin{itemize}
%	\item Fixiere $u\in V(G)\backslash K$ und $\emptyset \neq D'\subsetneq D$
...
\begin{frame}
	...
	\framesubtitle{Beweis}
	\begin{itemize}
	\item $V = V(G)\backslash K$, $v_i\in V$ und $D_i\subseteq K$, $|D_{i-1}| \leq |D_{i}|$
	\end{itemize}
...
\begin{frame}
	...
	\begin{alertblock}{Definition: Ganzzahlige Lineare Programmierungs Machbarkeit}
		Beim Ganzzahligen Linearen Programmierungs Machbarkeits Problem (Integer Linear Programming Feasibility Problem) bekommt man ein $p$ Variablen $x_1,x_2,...,x_p$ und eine Menge an $m$ Ungleichungen in der Form:\\
		\begin{tabular}{lc}
...
\begin{frame}
\frametitle{Ganzahlige Lineare Programmierung}
...
\begin{frame}
	...
	%\framesubtitle{Beweis}	
	\begin{itemize}
	\item Wir betrachten uns jetzt eine Ordnung $\pi_X$, so dass $X = \{u_1,u_2,...,u_k\}$ gilt.
	\begin{itemize}
		\item Dann gilt auch $\pi(u_1) < \pi(u_2) < ... < \pi(u_k)$.
	\end{itemize}	 
...
\begin{frame}
	\frametitle{Formulierung des Linearen Problems}
	%\framesubtitle{Beweis}	
	\begin{tabular}{lll}
